				...
				if filename!="." and filename!=".." and !(File.directory?(dynamic_path+"/"+filename)) and filename.index("powercat-ftrace")
					...
					while line=file.gets
						...
						if line[19]	#if the TIME field exists——this record is a complete recorde after protected mode
							...
							calltime=line[5].hex	#have existed in DLIST
							returntime=line[7].hex	#have existed in DLIST
							fname=line[9]
						...
						#puts returntime
							...
							if fdfile=="null"
								rsF=mydb.query("SELECT f_id FROM `#{$FDoption+"FDLIST"}` WHERE f_name='#{fname}'")	#Serch F function in the FDLIST by F_name
								rsF.each_hash do |rowF|
									fpoint=rowF['f_id']
									break	
								end
							else
								rsF=mydb.query("SELECT f_id FROM `#{$FDoption+"FDLIST"}` WHERE f_name='#{fname}' AND f_dfile='#{fdfile}'")	#Serch F function in the FDLIST by F_name, F_dfile
								rsF.each_hash do |rowF|
									fpoint=rowF['f_id']
								end
							end
							...
							if cdfile=="null"
								rsC=mydb.query("SELECT f_id FROM `#{$FDoption+"FDLIST"}` WHERE f_name='#{cname}'")	#Serch C function in the FDLIST by C_name
								rsC.each_hash do |rowC|
									cpoint=rowC['f_id']
									break
								end
							else
								rsC=mydb.query("SELECT f_id FROM `#{$FDoption+"FDLIST"}` WHERE f_name='#{cname}' AND f_dfile='#{cdfile}'")	#Serch C function in the FDLIST by C_name, C_dfile
								rsC.each_hash do |rowC|
									cpoint=rowC['f_id']
									break
								end
							...
							dlist_everyvalue.concat(["(#{fpoint}, #{cpoint}, '#{pid}', '#{tid}', '#{calltime}', '#{returntime}',  #{dept}),"])
...
#puts dlist_values
								...
								mydb.query("INSERT INTO `#{$option+"DLIST"}` (F_point, C_point, PID, TID, C_time, R_time,  Dept) VALUES#{dlist_values}")
#puts "INSERT INTO `#{$option+"DLIST"}` (F_point, C_point, PID, TID, C_time, R_time, RunTime) VALUES#{dlist_values}"
...
#							mydb.query("INSERT INTO `#{$option+"DLIST"}` (F_point, C_point, PID, TID, C_time, R_time, RunTime) VALUES(#{fpoint}, #{cpoint}, '#{pid}', '#{tid}', '#{calltime}', '#{returntime}', #{time})")	#DLIST中的Cd_address字段在动态跟踪数据中对应着AT位置的数据，目前此数据还没有
			...
			if dlist_everyvalue.size>0	#insert into DLIST , 1000 as a group
				...
				mydb.query("INSERT INTO `#{$option+"DLIST"}` (F_point, C_point, PID, TID, C_time,  Dept) VALUES#{dlist_values}")
				dlist_everyvalue.clear
			end
	...
	def Enter.dolist()
		...
		rs.each_hash do |row|
			...
			rsF = mydb.query("SELECT f_name, f_dfile FROM `#{$FDoption+"FDLIST"}` WHERE f_id=#{row['F_point']}") #search the F_dfile in the FDLIST by F_point
			rsF.each_hash do |rowF|
			...
			rsC = mydb.query("SELECT f_name, f_dfile FROM `#{$FDoption+"FDLIST"}` WHERE f_id=#{row['C_point']}") #search the C_dfile in the FDLIST by C_point
			rsC.each_hash do |rowC|
