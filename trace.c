...
{
	struct file *over_file = filp_open(file_name, O_RDWR | O_APPEND | O_CREAT, 0777); 
		
	mm_segment_t old_fs;
	old_fs = get_fs();
	set_fs(KERNEL_DS);
	//write to file
	over_file->f_op->write(over_file, buf, strlen(buf), &over_file->f_pos);
	set_fs(old_fs);
 ...
 */
/*static ssize_t
__tracing_read_pipe(struct file *file, char __user *ubuf,
		  size_t cnt, loff_t *ppos);
static ssize_t
...
{
	char * s = "LET'S START!";
	copy_to_user(ubuf, (void*)s, cnt);
	__tracing_read_pipe(file, ubuf, cnt, ppos);
	return cnt;
}*/
...
//	int sret;
	...
	buf_out_4 = kmalloc(DATA_PAGE_SIZE * 4, GFP_KERNEL);
	if (!buf_out_4) {
		printk("buf_out kmalloc fail!\n");
		goto out;
	}
	buf_4page = kmalloc(DATA_PAGE_SIZE * 4, GFP_KERNEL);
	if (!buf_4page) {
		printk("buf_out kmalloc fail!\n");
		goto out;
	}
